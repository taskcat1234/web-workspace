...
public class OrderServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * => 요청 전송 방식(GET), 사용자의 IP주소, 어떤 URL을 통해서 왔는지, 사용자가 입력한 값 등등...
		 * 
		 * 두 번쨰 매개 변수 httpServletRespoonse => 요청 처리 후 응답할 때 사용
		 * 
		 */
		//String queryString = request.getQueryString();
		//System.out.println(queryString);   -> num=1&product-name=123
		...
		/*
		 * xxx(사용자가 입력한 제품명)의 총 가격은 (총 결제해야하는 금액)원 입니다.
		 */
		...
		writer.println("<html>");
		writer.println("<head>");
		writer.println("<title> GET방식 응답입니다 </title>");
		writer.println("</head>");
